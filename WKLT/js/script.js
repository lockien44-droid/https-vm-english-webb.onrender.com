...
const renderer = new THREE.WebGLRenderer({
  ...
  alpha: true      // Bật chế độ nền trong suốt
});

// 设置颜色及其透明度 (Đặt màu và độ trong suốt - alpha = 0 là trong suốt)
...
// 轨迹球控制器 (Đã loại bỏ TrackballControls để tránh click chuột bị lỗi hiển thị)
...
new THREE.OBJLoader().load(
  ...
  (obj) => {
    ...
    // 基础网格材质 (Ẩn khối trái tim đen ở giữa)
...
const beat = { a: 0 };
const beatTimeline = gsap
  .timeline({
    repeat: -1,
    repeatDelay: 0.8, // Slowed down from 0.3
  });
  
beatTimeline.to(beat, {
    a: 0.5,
...
window.addEventListener('message', (event) => {
    if(event.data && event.data.type === 'toggleHeart') {
...
function render(a) {
  ...
  // Thêm hiệu ứng xoay cho group chứa toàn bộ các hạt và khối (quay chậm lại)
  if(!isPaused) {
      group.rotation.y += 0.003;
  }

  // controls.update(); // Bỏ update controls để chặn tương tác chuột
